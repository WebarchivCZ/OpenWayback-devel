...
=======
            <a href="<%= staticPrefix %>"><img src="<%= staticPrefix %>images/OpenWayback-banner.png" alt="logo: OpenWayback" /></a>
>>>>>>> fea8468e1186275e998c8062b1b5e80867c6067f
        </div>
        ...
        <div id="form">
            ...
            <div id="wbMeta">
                <p class="wbThis"><a href="<%= data.searchUrlForHTML %>"><%= data.searchUrlForHTML %></a> <%= fmt.format("BubbleCalendar.crawledInfo") %><strong><%= fmt.format("{0}x.",data.numResults) %></strong> <%= fmt.format("BubbleCalendar.crawledInfo2") %><a href="<%= data.firstResultReplayUrl %>"><%= fmt.format("{0,date,d. MMMMM yyyy}",data.firstResultDate) %></a>.</p>
			...
			if(count > 0) {
				...
				%><td>
                    <div class="date tooltip">
                        <div class="pop">
                            <h3><%= fmt.format("{0,date,d. MMMMM yyyy}",firstCaptureInDayDate) %></h3>
                            <p><%= count %>x archivováno</p>
                            <ul>
							<%
